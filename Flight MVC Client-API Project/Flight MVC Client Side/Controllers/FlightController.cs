    ...
    public class FlightController : Controller{
        ...
        public async  Task<ActionResult> SearchFlight(){
            ...
            if(cid == null){
                ...
                var destinationsTask =  GetDestinationFromApi();
                var sourcesTask =  GetSourcesFromApi();
    
                var destinations = await destinationsTask;
                var sources = await sourcesTask;

                ViewBag.Flightsources=new SelectList(sources);
                ViewBag.Flightdestinations=new SelectList(destinations);
                return View();
            }
        }

        private async Task<List<string>> GetDestinationFromApi(){
            
            List<string> l = new List<string>();
            using (var httpClient1 = new HttpClient())
            {
                using (var response1 = await httpClient1.GetAsync("http://localhost:5071/api/Flight/destinations")){
                    
                    var content1 = await response1.Content.ReadAsStringAsync();
                    l = JsonConvert.DeserializeObject<List<string>>(content1);
                }
        ...
        private async Task<List<string>> GetSourcesFromApi(){
            
            List<string> l = new List<string>();
            using (var httpClient1 = new HttpClient())
            {
                using (var response1 = await httpClient1.GetAsync("http://localhost:5071/api/Flight/sources")){
